...
const Stepper = React.forwardRef<HTMLDivElement, StepperProps>(
  ({ defaultValue = 0, value, onValueChange, orientation = "horizontal", className, ...props }, ref) => {
    ...
    const currentValue = React.useMemo(() => ({ activeStep: currentStep, setActiveStep, orientation }), [currentStep, setActiveStep, orientation]);

    return (
      <StepperContext.Provider value={currentValue}>
        <div
          ref={ref}
          className={cn(
...
const StepperItem = React.forwardRef<HTMLDivElement, StepperItemProps>(
  ({ step, completed = false, disabled = false, loading = false, className, children, ...props }, ref) => {
    ...
    const currentValue = React.useMemo(() => ({ step, state, isDisabled: disabled, isLoading }), [step, state, disabled, isLoading]);

    return (
      <StepItemContext.Provider value={currentValue}>
        <div
          ref={ref}
          className={cn(
